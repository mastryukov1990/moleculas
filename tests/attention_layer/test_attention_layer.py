...
from lib.preprocessing.models.molecul_graph_builder.dgl_graph import (
    MoleculeGraphBuilder, FEATURE_COLUMN,
)
...
def test_multy_head_attention_layer(molecules):
    ...
    atoms = list(itertools.chain.from_iterable([[a.GetSymbol() for a in get_molecule_from_smile(smile).GetAtoms()] for smile in molecules]))
    num_atoms = len(atoms)
    ...
    m_layer = MultiHeadAttentionLayerEdge(config)
